...
# --- 0. ★ ロギング設定 ---
def setup_logging(logfile='experiment_sw_spsa.log'):
    ...
    def aggregate_and_update(self, client_groups, epsilon, eta_B_server, compute_shapley_round, mc_iterations):
        ...
        for cid in client_groups['plus']:
            if cid in weights and cid in self.all_Rewards:
                w_i = weights[cid]
                R_i = self.all_Rewards[cid]
                R_plus_weighted_sum += w_i * R_i
                R_plus_weight_sum += w_i
        
        R_minus_weighted_sum, R_minus_weight_sum = 0.0, 0.0
        for cid in client_groups['minus']:
             if cid in weights and cid in self.all_Rewards:
                w_i = weights[cid]
                R_i = self.all_Rewards[cid]
                R_minus_weighted_sum += w_i * R_i
                R_minus_weight_sum += w_i

        R_plus = R_plus_weighted_sum / R_plus_weight_sum if R_plus_weight_sum > 0 else 0
        R_minus = R_minus_weighted_sum / R_minus_weight_sum if R_minus_weight_sum > 0 else 0
...
def run_main_training(config, all_datasets):
    logging.info(f"--- [パート1] Shapley-Weighted SPSA 版 ---")
...
if __name__ == "__main__":
    ...
    setup_logging(logfile='experiment_sw_spsa.log')
